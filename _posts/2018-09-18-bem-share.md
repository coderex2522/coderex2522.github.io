...
BEM是由[Yandex](https://yandex.com/)团队（俄罗斯最著名的互联网企业，旗下Yandex浏览器在当地市场占用率超过Chrome）提出的存储命名方法论，意思就是块(block)、元素(element)、修饰符(modifier),BEM思想让css类更加透明更加有意义。
...
命名空间填补了BEM的遗憾，BEM加命名空间，解决了css难维护，难阅读，难复用的问题，提高了代码的通用性，也降低了css与html的耦合度。BEM传达的是一种思想，一种规范，让代码更具备可读性和可维护性；准确地说，其中BEM应该是一个完整的存储开发理论，不仅限于css。